...
pub(crate) fn serve_command(stream: &mut TcpStream, tpm: &Box<dyn Driver>) -> Result<bool> {
...
/// to `tpm`.
fn handle_cmd(cmd: TcpTpmCommands, stream: &mut TcpStream, tpm: &Box<dyn Driver>) -> Result<()> {
    ...
    match cmd {
        TcpTpmCommands::RemoteHandshake => {
            ...
            let _ = stream.write(&SERVER_VERSION.to_be_bytes())?;
            // TODO Make TpmEndpointInfo a bitfield and send
            let _ = stream.write(&CFG.to_be_bytes())?;
...
fn handle_send(stream: &mut TcpStream, tpm: &Box<dyn Driver>) -> Result<()> {
    let mut locality = [0u8; 1];
    let mut sz = [0u8; 4];
    let _ = stream.read(&mut locality)?;
    let _ = stream.read(&mut sz)?;
    ...
    if let Ok(res) = tpm.execute_command(&cmd) {
        let _ = stream.write(&(res.len() as u32).to_be_bytes())?;
